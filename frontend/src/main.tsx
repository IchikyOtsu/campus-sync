...
function App() {
  ...
  onMount(async () => { if (!supabase) return; const { data } = await supabase.auth.getSession(); setSession(data.session); if (data.session) cleanAuthUrl(); const subscription = supabase.auth.onAuthStateChange((_event, nextSession) => { setSession(nextSession); if (nextSession) cleanAuthUrl(); }); onCleanup(() => subscription.data.subscription.unsubscribe()); });
