...
function ProgramSection(props: ProgramProps) {
  ...
  const removeProgram = async () => { const program = detail(); if (!program) return; setLoading(true); try { await api(`/me/programs/${program.id}`, { method: 'DELETE' }); setDetail(undefined); await props.onChanged(); setMessage('Programme supprimé. Les cours déjà présents dans votre PAE sont conservés.'); } catch (error) { setMessage(apiErrorLabel(error, 'Impossible de supprimer le programme')); } finally { setLoading(false); } };
