...
import { api, apiErrorLabel, configurationError, supabase } from './api/client';
import './style.css';
...
type TimeEditCourse = { code: string; name: string; external_id: string; academic_year: string; institution: 'ulb' | 'uclouvain' | 'unamur' | 'esi'; object_type: string };
...
function CourseSearch(props: CourseSearchProps) {
  ...
  return <section id="course-search" class="course-search"><div><label>AJOUT AUTOMATIQUE</label><h2>Ajouter un cours au PAE</h2></div><div class="course-fields"><label>Source<select disabled><option>TimeEdit public</option></select></label><label>Année académique<select disabled><option>{academicYear}</option></select></label><label>Code ou nom<input value={query()} placeholder="ELEC-H550" onInput={e => setQuery(e.currentTarget.value)} onKeyDown={e => e.key === 'Enter' && search()} /></label><button disabled={loading()} onClick={search}>{loading() ? 'Recherche…' : 'Rechercher'}</button></div><Show when={message()}><p class="search-message">{message()}</p></Show><For each={results()}>{course => <article class="course-result"><div><span>{institutionLabel(course.institution)} · {course.academic_year}</span><h3>{course.code}</h3><p>{course.name}</p></div><button disabled={loading() || alreadyAdded(course)} onClick={() => add(course)}>{alreadyAdded(course) ? 'Déjà dans le PAE' : 'Ajouter au PAE'}</button></article>}</For></section>;
}
...
function App() {
  ...
  const refreshDashboard = async () => { await Promise.allSettled([refetchPae(), refetchCourses(), refetchEvents(), refetchConflicts()]); };
  ...
  onMount(() => { const syncPage = () => setPage(window.location.pathname === '/calendrier' ? 'calendar' : 'pae'); window.addEventListener('popstate', syncPage); onCleanup(() => window.removeEventListener('popstate', syncPage)); });
  const view = () => {
    ...
    return <div class="app"><header><div class="logo">✣ campus-sync</div><nav><button classList={{ active: page() === 'pae' }} onClick={() => navigate('pae')}>Mon PAE</button><button classList={{ active: page() === 'calendar' }} onClick={() => navigate('calendar')}>Calendrier</button></nav><span class="profile">{profile()?.display_name || user.user.email || 'Compte connecté'}</span><button onClick={() => supabase?.auth.signOut()}>Déconnexion</button></header><main><Show when={page() === 'calendar'}><CalendarPage events={events} loading={() => events.loading} error={() => events.error} /></Show><Show when={page() === 'pae'}><section class="hero"><div><label>MON ESPACE</label><h1>Bonjour {profile()?.display_name || user.user.email}</h1><p>Votre calendrier interuniversitaire.</p></div></section><CourseSearch courses={courses} onAdded={refreshDashboard} onAddedOffering={(nextPae, offering) => { mutatePae(nextPae); mutateCourses(current => current?.some(item => item.id === offering.id) ? current : [...(current || []), offering]); }} /><Show when={notice()}><p class="notice">{notice()}</p></Show><section class="courses"><h2>Mon PAE · {year()}</h2><p>{pae()?.course_count ?? 0} cours</p><Show when={pae.error}><p class="search-message">{apiErrorLabel(pae.error, 'Impossible de charger le PAE')}</p></Show><Show when={courses.error}><p class="search-message">{apiErrorLabel(courses.error, 'Impossible de charger les cours du PAE')}</p></Show><Show when={courses.loading}><p>Chargement des cours…</p></Show><Show when={!courses.loading && !courses.error && !courses()?.length}><p>Votre PAE est vide.</p><button onClick={() => document.getElementById('course-search')?.scrollIntoView()}>Ajouter un cours</button></Show><For each={courses() || []}>{item => <div>{item.course.code} — {item.course.name}<small> · {item.course.institution.name}</small><button onClick={async () => { try { await api(`/me/pae/courses/${item.id}`, { method: 'DELETE' }); mutateCourses(current => current?.filter(course => course.id !== item.id)); mutatePae(current => current ? { ...current, course_count: Math.max(0, current.course_count - 1) } : current); await refreshDashboard(); setNotice(`${item.course.code} a été retiré du PAE.`); } catch (error) { setNotice(apiErrorLabel(error, `Impossible de retirer ${item.course.code}`)); } }}>Retirer</button></div>}</For></section><section class="status"><div><b>Source des horaires</b><span>TimeEdit public · ULB, UCLouvain, UNamur et ESI</span></div><div class="conflict"><b>{conflicts()?.length || 0} conflit(s)</b><span>Calculés à partir de votre PAE.</span></div></section></Show></main></div>;
  };
  return <>{view()}</>;
}
