...
type CourseSearchProps = { onAdded: () => Promise<void> };
...
function CourseSearch(props: CourseSearchProps) {
  ...
  const add = async (course: TimeEditCourse) => { setLoading(true); setMessage(''); try { const result = await api<{ pae: PAE }>('/institutions/ulb/courses/add', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code: course.code, external_id: course.external_id, academic_year: course.academic_year }) }); await props.onAdded(); setMessage(`${course.code} a été ajouté au PAE (${result.pae.course_count} cours).`); } catch (error) { setMessage(apiErrorLabel(error, `Impossible d’ajouter ${course.code}`)); } finally { setLoading(false); } };
