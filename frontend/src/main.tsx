...
function App() {
  ...
  const [pae, { refetch: refetchPae }] = createResource(() => profile() ? api<PAE>('/me/pae') : Promise.resolve(undefined));
  const [events, { refetch }] = createResource(() => profile() ? api<Event[]>('/me/pae/events') : Promise.resolve([]));
  const [courses, { refetch: refetchCourses }] = createResource(() => profile() ? api<any[]>('/me/pae/courses') : Promise.resolve([]));
  ...
  const refreshDashboard = () => { void refetchPae(); void refetch(); void refetchCourses(); void refetchConflicts(); void refetchUserPrograms(); };
